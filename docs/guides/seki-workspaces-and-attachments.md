...
- OGS rank, error, auth, primitive sanitizer, user/player, state clone, and
  automatch option/payload helpers are extracted under `src/ogs/` while
  `src/ogs.js` remains the public client facade.
